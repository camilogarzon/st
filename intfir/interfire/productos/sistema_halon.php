			<h1 class="pageTitle">Sistema de Extinci—n de Incendios Alternativos al Hal—n</h1>
			
			<div id="tabs" class="tabs2">
                <ul>
                    <li><a href="#tabs-1">Registro</a></li>
                    <li><a href="#tabs-2">Rev. Mensual</a></li>
                    <li><a href="#tabs-3">Rev. Semestral</a></li>
                    <li><a href="#tabs-4">Rev. Anual</a></li>
                    <li><a href="#tabs-5">Rev. Quinquenal</a></li>
                </ul>
                
                <div id="tabs-1">
                
                <?php include("sistema_halon/registro.php") ?>
                
                </div>
                
                <div id="tabs-2">
                
                <?php include("sistema_halon/rev_mensual.php") ?>
                
                </div>
                
                <div id="tabs-3">
                
                <?php include("sistema_halon/rev_semestral.php") ?>
                
                </div>
                
                <div id="tabs-4">
                
                <?php include("sistema_halon/rev_anual.php") ?>
                
                </div>
                           
                <div id="tabs-5">
                
                <?php include("sistema_halon/rev_quinquenal.php") ?>
                
                </div>
                
			</div>